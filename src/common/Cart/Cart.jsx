  ...
  useEffect(() => {
    const FecthData = async () => {
      try {
        ...
        const response = await ProductAPI.getdiscount();
              ...
              return (
                <div className="cart-list product d_flex" key={item.productId}>
                  ...
                  <div className="cart-details">
                    ...
                    {
                      discount.map((values) => {
                        if (values.discountId == item.discountId) {
                          const ab = values.discountPercent * item.price;
                          totalPrice += (item.price - ab) * item.qty;
                          return <h4>${item.price - ab} * {item.qty}<span>{(item.price - ab) * item.qty}</span> </h4>
                        }
                      })
                    }
                    {/* <h4>
